...
class ServiceLayerController extends Controller
{
    ...
    public function isAuthorized($serviceActionRegistryID){
      if($this->user()){
        ...
        $roleAccessList = [];
        $userAccessList = [];
        $userAccessList = (new App\Models\UserAccessList())->where('user_id', $user['id'])->where('service_action_registry_id', $serviceActionRegistryID)->get()->toArray();
        if($user['role_list'] && count($user['role_list'])){
          $roleAccessList = (new App\Models\RoleAccessList())->whereIn('role_id', $user['role_list'])->where('service_action_registry_id', $serviceActionRegistryID)->get()->toArray();
        }
    ...
    public function getService($link){
      ...
      */
      if($serviceActionRegistry){
        $serviceAction = (new App\Models\ServiceAction())
